...
#####plot muticum segment assignments
dat <- read.table(paste(prefix, '_segment_assignments_1Mb.tsv', sep = ''))
colnames(dat) <- c("chr", "position", "cov", "genotype")
dat$position <- dat$position / 1000000

plot <- ggplot(data = dat, aes(x=position, y=cov, fill = genotype, colour = genotype)) +
  geom_point(size=0.5) +
